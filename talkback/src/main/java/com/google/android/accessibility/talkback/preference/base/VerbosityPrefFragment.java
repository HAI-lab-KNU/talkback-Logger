...
public class VerbosityPrefFragment extends TalkbackBaseFragment {
  ...
  private ArrayList<Preference> collectDetailedPreferences() {
    ...
    for (int p = 0; p < prefGroup.getPreferenceCount(); p++) {
      ...
      if (preference != null) {
        ...
        if (key != null) {
          Object value = getPreferenceValue(sharedPreferences, key);
          Log.d("CHECK! VerboseSetting", "Preference: " + key + " = " + value);
        }
      }
    }
    return detailedPrefs;
  ...
  private Object getPreferenceValue(SharedPreferences sharedPreferences, String key) {
